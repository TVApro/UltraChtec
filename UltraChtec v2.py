...
class FinderField():
    ...
    def finder_field():
        ...
        key_entry = Entry(finderfield, width=80, font = (SIZE))
...
if __name__ == '__main__':
    ...
    filemenu.add_command(label="Открыть...", command = StartWindow.path_input, font = (SIZE))
    filemenu.add_command(label="Сохранить", command = StartWindow.save_file_simple, font = (SIZE))
    filemenu.add_command(label="Сохранить как...", command = StartWindow.save_file_as, font = (SIZE))
    filemenu.add_command(label="Выход", command = StartWindow.exit_file, font = (SIZE))

    editmenu = Menu(mainmenu, tearoff=0)
    editmenu.add_command(label="Просмотр содержимого", command = StartWindow.show_file, font = (SIZE))
    editmenu.add_command(label='Отменить действие', command = undo, font = (SIZE))
    editmenu.add_command(label='Вернуть действие', command = redo, font = (SIZE))
    editmenu.add_command(label='Скопировать текст', command = all_text_copy, font = (SIZE))
    editmenu.add_command(label="Увеличить текст", command = increase, font = (SIZE))
    editmenu.add_command(label="Уменьшить текст", command = decrease, font = (SIZE))
    editmenu.add_command(label="Очистить поле", command = StartWindow.clear, font = (SIZE))

    specmenu = Menu(mainmenu, tearoff=0)
    specmenu.add_command(label="Анализ генома", command = StartWindow.input_assembly, font = (SIZE))
    specmenu.add_command(label="Простой поиск", command = TextFindField.text_find_field, font = (SIZE))
    specmenu.add_command(label="Поиск по аннотациям", command = FinderField.finder_field, font = (SIZE))

    opcmenu = Menu(mainmenu, tearoff=0)
    opcmenu.add_checkbutton(label="Запретить перезапись файла", onvalue=1, offvalue=0, variable=isblocked, font=(SIZE))
    opcmenu.add_checkbutton(label="Режим свитка", onvalue=1, offvalue=0, variable=islogfile, font=(SIZE))

    helpmenu = Menu(mainmenu, tearoff=0)
    helpmenu.add_command(label="Помощь", command = StartWindow.help, font = (SIZE))
    helpmenu.add_command(label="О программе", command = StartWindow.spravka, font = (SIZE))

    mainmenu.add_cascade(label="Файл", menu=filemenu, font = (SIZE))
    mainmenu.add_cascade(label="Правка", menu=editmenu, font = (SIZE))
    mainmenu.add_cascade(label="Возможности", menu=specmenu, font = (SIZE))
    mainmenu.add_cascade(label="Режимы", menu=opcmenu, font = (SIZE))
    mainmenu.add_cascade(label="Справка", menu=helpmenu, font = (SIZE))

    # фреймы
    path_input = LabelFrame(window, text="Путь поиска файла", labelanchor=NE, font=(SIZE), padx=5, pady=5)
    ...
    features_list = Text(output, xscrollcommand = on_xscrollcommand, yscrollcommand = on_yscrollcommand, wrap = NONE, undo = True, font = ("Liberation Mono", SIZE))
    features_list.focus_set
